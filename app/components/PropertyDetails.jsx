    ...
    <main>
      <div className="bg-white p-6 rounded-lg shadow-md text-center md:text-left">
        ...
        <div className="text-gray-500 mb-4 flex align-middle justify-center md:justify-start">
          <FaMapMarker className="inline mr-2 text-red-600" />
          <p className="text-orange-700">{property.location.city} {property.location.street}, {property.location.state} {property.location.zipcode}</p>
        </div>
        ...
        {property.rates.nightly ? (
          `$${Number(property.rates.nightly).toLocaleString()}`
        ...
        {property.rates.weekly ? (
          `$${Number(property.rates.weekly).toLocaleString()}`
        ) : (<FaTimes className="text-red-700"/>)}
                </div>
          </div>
        ...
        {property.rates.monthly ? (
          `$${Number(property.rates.monthly).toLocaleString()}`
        ) : (<FaTimes className="text-red-700"/>)}
                </div>
          </div>
        </div>
      ...
      <div className="bg-white p-6 rounded-lg shadow-md mt-6">
        ...
        <div className="flex justify-center gap-4 text-blue-500 mb-4 text-xl space-x-9">
          ...
          <p>
            ...
            {property.size} <span className="hidden sm:inline">sqft</span>
          </p>
        </div>
